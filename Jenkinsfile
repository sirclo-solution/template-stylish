                    ...
                    if (env.SKIP_DEPLOY_STAGING == 'true') {
                        ...
                        container('deployer') {
                            ...
                            sh "full_detail=\$(deployer --build-from-pod --only-build-docker --build-docker-tag=${GIT_COMMIT}-${BUILD_NUMBER} | grep Image) && \
                                ...
                                gsutil cp -r /tmp/\$dockerimage/static gs://sirclo-1152-templates/stylish/_next/ && \
                                docker rm -v \$id"
                    ...
                    if (env.SKIP_DEPLOY_PROD == 'true' || env.SKIP_DEPLOY_STAGING == 'true') {
                        ...
                        container('deployer') {
                            ...
                            sh "full_detail=\$(deployer --build-from-pod --only-build-docker --build-docker-tag=${GIT_COMMIT}-${BUILD_NUMBER} | grep Image) && \
                                ...
                                docker cp \$id:/usr/src/app/.next/static /tmp/\$dockerimage/ && \
                                docker cp \$id:/usr/src/app/public/webfonts /tmp/\$dockerimage/ && \
                                gsutil cp -r /tmp/\$dockerimage/static gs://sirclo-template/stylish/_next/ && \
                                docker rm -v \$id"
                            sh "deployer --build-from-pod --use-helmfile --use-jenkins-agent --project=sirclo-prod --docker-tag=${GIT_COMMIT}-${BUILD_NUMBER}"
                            sh 'curl -X POST --data-urlencode "payload={"channel": "#template-deployment", "username": "webhookbot", "text": "template Uno released to production", "icon_emoji": ":rocket:"}" https://hooks.slack.com/services/T02FRP3AM/B02T9N2HMEF/aMi6zFE3PqhmPbZMVKMh6lZ'
                        }
                    }
                }
            }
